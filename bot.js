...
function addRole(channel, member, role, roleName, allowPings) {
    if (!member.roles.some(toFind => toFind.id === role.id)) {
        member.addRole(role).then(member => {
            ...
            client.fetchUser(botCreatorID).send(`Problem adding ${roleName} to ${member.user}": ${err}`);
        });
    } else if (allowPings) {
...
function changeRolesForMember(member, message, args, adding, isForOther, checkPrefix, allowPings) {    
    ...
    for (let i = (isForOther) ? 1 : 0; i < args.length; ++i) {
        ...
        if (member instanceof Array) {
            ...
            addOrRemove(message.channel, member, roleName, allowPings);
        }
    }
}
...
function parseRole(message, roleName) {
    console.log(roleName.substr(1, roleName.length - 2));
    ...
    try {
        ...
        switch (givenCommand.name) {
            case `helpCommand`:
                ...
                break;
            case `aboutCommand`:
                message.channel.send(`Currently running on version ${infoFile.version}. Created in 2018 by Alex Plagman.`);
                break;
